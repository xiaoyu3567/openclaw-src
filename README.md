...
## Fork customizations (xiaoyu3567/openclaw-src)

This fork tracks OpenClaw `2026.2.27` and adds custom Control UI + session workflow capabilities for personal operations.

### Session management enhancements (Control UI)

- Added chat topbar session actions:
  - Create (`+`), delete (trash), and refresh controls in the same interaction zone.
  - Create supports full key, short suffix mode (`agent:main:xxx`), and random suffix generation.
  - Newly created session auto-switches to reduce manual steps.
- Improved session deletion behavior:
  - Match target by list index, full session key, or short key tail.
  - Removed silent failures; deletion now returns explicit success/error feedback.
- Control UI uses the `ui` gateway client mode so dashboard-triggered session mutations are accepted by gateway policy.

### Chat input behavior optimization

- Input/send shortcuts changed to reduce accidental sends:
  - `Enter` inserts newline.
  - `Ctrl/Cmd+Enter` sends message.
- Removed composer-level `New session` button to avoid accidental resets during drafting.

### Backend per-session thinking timer

- Added persistent backend session fields:
  - `thinkingStartedAt`
  - `thinkingRunId`
- Exposed thinking state via `sessions.list` for UI consumption.
- Added topbar per-session live label:
  - `idle`
  - `thinking: HH:MM:SS`
- Timer remains consistent across session switching, page refresh, and multi-device view because state source is server-side session metadata.

### Mobile UI refinement

- Fixed chat controls that were present in DOM but hidden by mobile header media rules.
- Updated mobile chat header/control wrapping and interaction spacing.
- Compacted mobile chat spacing:
  - `chat` layout gap reduced to `0`
  - chat card inner padding removed for denser reading/operation space.

### Provider usage panel (new)

- Added multi-provider usage monitoring panel in Usage tab.
- Supports adding multiple provider entries with:
  - name
  - base URL
  - API key
  - refresh interval and timeout
- Added provider card metrics and status display:
  - remaining quota and plan info
  - daily/weekly/monthly usage progress
  - RPM/TPM, latency, and last refresh timestamp
  - raw JSON detail expansion for troubleshooting.
