...
async function fetchSub2ApiUsageViaGateway(params: ProviderFetchParams) {
  const baseUrl =
    typeof params.baseUrl === "string" ? params.baseUrl.trim().replace(/\/$/, "") : "";
  const apiKey = typeof params.apiKey === "string" ? params.apiKey.trim() : "";
  const timeoutMsRaw =
    typeof params.timeoutMs === "number" ? params.timeoutMs : Number(params.timeoutMs ?? 12000);
  const timeoutMs = Number.isFinite(timeoutMsRaw)
    ? Math.max(2000, Math.floor(timeoutMsRaw))
    : 12000;
  if (!baseUrl || !apiKey) {
    ...
    if (!response.ok) {
      const detail =
        payload && typeof payload === "object" && payload !== null && "message" in payload
          ...
          : null;
      const message =
        typeof detail === "string" && detail.trim() ? detail : `HTTP ${response.status}`;
...
export const usageHandlers: GatewayRequestHandlers = {
  ...
  "usage.provider.fetch": async ({ respond, params }) => {
    try {
      const type = typeof params?.type === "string" ? params.type : "sub2api";
      if (type !== "sub2api") {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, `unsupported provider type: ${type}`),
        );
        return;
      }
      const payload = await fetchSub2ApiUsageViaGateway(params);
