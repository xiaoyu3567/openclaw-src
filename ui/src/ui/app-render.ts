...
function formatTopbarThinkingElapsed(ms: number): string {
...
function resolveTopbarThinkingStartedAt(state: AppViewState): number | null {
  ...
  if (state.chatRunId && typeof state.chatStreamStartedAt === "number") {
    return state.chatStreamStartedAt;
  }
  return null;
}
...
export function renderApp(state: AppViewState) {
  ...
  const thinkingStatusClass = resolveTopbarThinkingClass(state);
  const thinkingStatusLabel = resolveTopbarThinkingLabel(state);
    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      <header class="topbar">
        ...
        <div class="topbar-status">
          <div class="pill topbar-thinking-pill" title="Per-session agent state">
            <span class="statusDot ${thinkingStatusClass}"></span>
            <strong class="topbar-thinking-pill__text">${thinkingStatusLabel}</strong>
          </div>
          <div class="pill">
        ...
        ${
          state.tab === "files"
            ? renderFiles({
                ...
                contextMenuOpen: state.filesContextMenuOpen,
                contextMenuTargetPath: state.filesContextMenuTargetPath,
                contextMenuPosition: state.filesContextMenuPosition,
                previewOpen: state.filesPreviewOpen,
                previewPath: state.filesPreviewPath,
                previewKind: state.filesPreviewKind,
                ...
                previewImageMode: state.filesPreviewImageMode,
                previewMarkdownMode: state.filesPreviewMarkdownMode,
                ...
                deleteError: state.filesDeleteError,
                ...
                onSelectPath: (path) => state.selectFilesPath(path),
                onDownload: (path) => void state.downloadFile(path),
                onPreview: (path) => void state.previewFile(path),
                onDelete: (path) => state.requestDeleteFile(path),
                onClosePreview: () => state.closeFilesPreview(),
                onSetPreviewDockMode: (mode) => state.setFilesPreviewDockMode(mode),
                onSetPreviewImageMode: (mode) => state.setFilesPreviewImageMode(mode),
                onSetPreviewPanelSize: (width, height) =>
                  state.setFilesPreviewPanelSize(width, height),
                ...
                onFileLongPress: (path, position) => state.openFilesContextMenu(path, position),
                onCloseContextMenu: () => state.closeFilesContextMenu(),
              })
            : nothing
        }
        ...
        ${
          state.tab === "chat"
            ? renderChat({
                ...
                refineResultKind: state.chatRefineResultKind,
                refineResultMessage: state.chatRefineResultMessage,
                canRefine: state.connected && state.chatMessage.trim().length > 0,
                canUndoRefine: Boolean(state.chatRefineLastOriginal),
                quickToolsOpen: state.quickToolsOpen,
                quickToolRunning: state.quickToolRunning,
                quickResultText: state.quickResultText,
                quickResultError: state.quickResultError,
                uploadRunning: state.chatUploadRunning,
                uploadProgress: state.chatUploadProgress,
                uploadError: state.chatUploadError,
                atPickerOpen: state.atPickerOpen,
                atPickerQuery: state.atPickerQuery,
                atPickerEntries: state.atPickerEntries,
                ...
                onToggleQuickTools: () => state.toggleQuickTools(),
                onRunQuickSummary: () => void state.handleRunQuickSummary(),
                onRunQuickTodos: () => void state.handleRunQuickTodos(),
                onCopyQuickResult: () => void state.handleCopyQuickResult(),
                onCloseQuickResult: () => state.handleCloseQuickResult(),
                onUploadFile: (file) => void state.handleUploadFile(file),
                onAtPickerQueryChange: (query) => void state.handleAtPickerQueryChange(query),
                onAtPickerSelect: (entry) => state.handleAtPickerSelect(entry),
                onAtPickerClose: () => state.handleAtPickerClose(),
