...
function handleTerminalChatEvent(
  ...
  const isTerminalPayload =
    payloadState === "final" || payloadState === "error" || payloadState === "aborted";
  const isTerminalCurrentState = state === "final" || state === "error" || state === "aborted";
  if (!isTerminalPayload && !isTerminalCurrentState) {
    return;
  }

  if (isTerminalCurrentState) {
    resetToolStream(host as unknown as Parameters<typeof resetToolStream>[0]);
